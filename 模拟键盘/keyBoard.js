...
function drawKeyboard(type) {
    $("#keyboardNum").empty(); // empty() 方法从被选元素移除所有内容，包括所有文本和子节点
    ...
    $("div[name='key']").hover(function() {
        ...
        switch (thisValue) {
            case "": //"
                ...
                break;
            case "Shift":
                $("#shift").prop("checked", $("#shift").prop("checked") == true ? false : true);
                if ($("#shift").prop("checked") == true) {
                    drawKeyboard("upper")
                } else {
                    if ($("#capsLock").prop("checked") != true) {
                        drawKeyboard("lower");
                    }
                }
                break;
            case "Caps Lock":
                $("#capsLock").prop("checked", $("#capsLock").prop("checked") == true ? false : true);
                $("#capsLock").prop("checked") == true ? drawKeyboard("upper") : drawKeyboard("lower");
...
$(function() {
    var divKeyBoard = '<div id="keyboard" class="keyboard"><div id="keyboardHead"><div><input id="shift" type="checkbox"/>Shift</div><div><input id="capsLock" type="checkbox"/>Caps Lock</div><div id="close" style="border:1px solid black; float:right; width:20px; height:20px;line-height:20px; cursor:pointer;">X</div></div><div id="keyboardNum"></div><div id="keyboardLetterQ"></div><div id="keyboardLetterA"></div><div id="keyboardLetterZ"></div><div id="keyboardSpaceBar"></div></div>';
    $("#keyboardParent").append(divKeyBoard);
    drawKeyboard("lower"); // 绘制小写键盘
    addKeydownEvent();
    $("#keyboard").css("visibility", "hidden");
